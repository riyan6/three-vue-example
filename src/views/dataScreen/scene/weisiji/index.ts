...
import { Ref } from "vue";
import wsjPath from "../../../../../public/models/wsj.glb";
...
export default function () {
  ...
  const init = (sceneRef: Ref<HTMLDivElement>) => {
    ...
    camera.position.set(-790, 326, 475);
    camera.rotation.set(-0.74, -1, -0.67);
    ...
    orbitControls.target.set(85, -1, 120);
    ...
    gltfLoader.load(wsjPath, (obj) => {
      ...
      model.position.y -= modelHeight / 1.3;

      const sprite = createCanvasSprite(["机台名称：喂丝机3#", "当前牌号：牌号B", "运行状态：运行"]);
